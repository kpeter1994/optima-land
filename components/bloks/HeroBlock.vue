...
<template>
  <div class="w-full min-h-screen lg:h-screen flex flex-col lg:flex-row relative">
    <header class="absolute w-full px-4 py-4 flex justify-between items-center">
      ...
      <a :class="`bg-${props.color}-950 font-bold text-lg text-white px-6 py-2 rounded-full shadow`"
         href="https://optimatagsag.hu/register">Regisztrálok</a>
    </header>

    <div :class="`bg-${props.color}-950 lg:h-full lg:w-7/12 bg-[url('/image/lines.svg')] flex justify-center`">
      <div class="max-w-4xl mt-40 lg:mt-28 2xl:mt-40 text-white px-4">
        <h1 v-html="hero.title" class="text-white text-4xl lg:text-6xl xl:text-7xl 2xl:text-8xl font-bold text-center lg:text-left">
        ...
        <div class="mt-6 mb-12 flex gap-3 flex-col lg:flex-row relative z-20">
    ...
    <div class="lg:absolute -bottom-40 xl:-bottom-20 2xl:-bottom-12 3xl:bottom-0 left-0 lg:w-9/12 bg-white pt-10 pb-6 px-4 lg:px-10">
      <div class="lg:flex w-full">

        <div v-for="card in hero.Card" class="lg:w-1/3 px-3 mt-3">
          <img class="w-12 mb-3" :src="card.image.data.attributes.url" :alt="card.image.data.attributes.alternativeText">
